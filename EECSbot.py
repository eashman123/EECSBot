...
def valinval(val_list, t_string):
    matches = [val for val in val_list if val in t_string]
    return matches
...
async def on_message(message):
    if message.author == client.user:
        return
    elif 'meme' in str(message.channel):
        if 'eecs' in message.content.lower():
            eecs_copypasta = ['http://eecseecs.com/meme.html',
                              '''As a member of the EECS community and a part of one of these organizations this is highly offensive. CS classes at UC Berkeley make it their goal to give students a place to feel comfortable as well as better the community. Trivializing their work by comparing specific classes to the humanities is absurd and beyond inaccurate. Making the claim that CS classes are cliques is demeaning the EE and CS they are founded on. This clearly is a stab at a community on campus that does nothing but support the rest of the student body.''',
                              '''Dude, this is a serious discord, and should be treated as such. Thousands of students are busting their asses off at this school, and you're making a mockery not only of the students who go to class day after day, paying thousands of dollars for the best education in the world. You can take your half-assed memes about EECS and Guy Fieri to leddit.''',
...
Finally they came for me and the rest of the EECS majors were too socially awkward to speak for me''']  # im a horrible person
        ...
        elif 'cs' in message.content.lower():
...
There really is a CS and these people are majoring in it, but it is just another layer on top of a more nuanced view of computing. CS is the theory of computation: the field dealing with abstract algorithms and data structures necessary to implement certain logic. CS is a necessary field, but its mathematical theory is useless by itself; it can only be put into practice with the help of microchips designed by electrical engineers to transform analog signals into discrete logic. Practical CS is normally reliant upon on a computer built upon layers and layers of circuitry. The whole field is basically CS built on top of EE, or EECS. All the so-called CS majors are really abstracted EE majors.'''
            ...
            competing_colleges = ['caltech', 'stanford', 'harvard', 'ucla', 'ucsd']
            private_copy = [
                'Berkeley is better than Stanford, Caltech, and Harvard by a mile. Idc what anyone says. People at Stanford couldn\'t cut it at Berkeley because nobody is getting coddled here. Let\'s be honest. The only thing Stanford has is money. Berkeley is better at everything else. By a wide margin. Ask any expert.',
                'I know not everyone who\'s at Cal is on the level of the average Stanford or Caltech student, but like I said, blame the state mandates that force us to take in mediocre transfer students who would have NEVER made it if they had applied the REAL way. Back in the 70s no department at berkeley was ranked less than 3rd, but then morons like reagan decided that the top university on the planet didn\'t need to be funded anymore. So we dropped off a bit, but are still on top despite. Pound for pound, Berkeley is better than Harvard.'
            ]
            public_copy = [
                'Less people apply to Berkeley because they realize how hard it is to get into Berkeley so they don\'t even try. Plus LA is so big. That means they have a lot of students to cherry pick from because of the perceptions of the 2 schools. But they also get a lot of mediocre students applying to because they figure "what the hell, ucla is lot easier than berkeley, i might as well apply cuz i know i wont get into berkeley." So more people take a chance at USCD or UCLA because they know it\'s easier. You have to be special to get into Cal, but to get into UCLA or UCSD? A lot easier. Trust me. No other UC is on the level of Cal, those are just facts.'
            ]
